...
{
public:
  explicit TranslateDlg( QWidget* = 0, Qt::WindowFlags flags = 0, bool = true );
  ~TranslateDlg();
  int                   getMode() const;
...
: QFileDialog( parent, flags )
{
  ...
  if( grid )
  {
    ...
    QLabel *label = new QLabel("STEP mode");
    grid->addWidget(label, row, 0, 1,1);
    grid->addWidget( paramGroup, row, 1, 1, columns-1 ); // make combobox occupy 1 row and 3 columns starting from 1
    grid->update();
  }
}
...
QString Translate::selectFileName( const int format, const bool import )
{
  ...
  {
    // TODO: fix selectedFilter()
    //QString selFilter = theDlg->selectedFilter(); // original
    QString selFilter = theDlg->selectedNameFilter(); // NFETTE
...
TranslateDlg* Translate::getDialog( const int format, const bool import )
{
  ...
  // TODO: fix setFilters()
  // myDlg->setFilters( filters ); // original
  myDlg->setNameFilters( filters ); // NFETTE
  ...
  if ( !import )
  {
    ...
    {
      case FormatSTEP:
      myDlg->addMode( STEPControl_AsIs, tr("As is"));
