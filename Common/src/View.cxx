...
void View::Popup( const int /*x*/, const int /*y*/ )
{
  ...
  QWidget* w = (QWidget*)ws->activeSubWindow();
#else
...
MyRubberBand::MyRubberBand(Shape s, QWidget * p)
  : QRubberBand(s, p)
{
}

void MyRubberBand::paintEvent(QPaintEvent *pe) {
  QPainter painter;
  QPen pen(Qt::red, 6);
...
{ 
  ...
  if ( !myRectBand ) 
  {
    ...
    //QPalette myPalette = myRectBand->palette();
    //QColor color(Qt::blue);
    //color.setAlpha(80);
    //myPalette.setBrush(QPalette::Highlight, QBrush(color));
    //myRectBand->setPalette(myPalette);
  ...
  else if ( m_IsVisible && !Draw ) // move or up  : erase at the old position
  {
    myRectBand->hide();
    //delete myRectBand;
    myRectBand->deleteLater();
    myRectBand = NULL;
    m_IsVisible = false;
  }
