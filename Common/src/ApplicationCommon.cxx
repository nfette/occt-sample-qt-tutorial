...
void ApplicationCommonWindow::windowsMenuAboutToShow()
{
  ...
  connect( a, SIGNAL( triggered() ), stWs, SLOT( cascadeSubWindows() ) );
  myWindowPopup->addAction( a );
  ...
  connect( a, SIGNAL( triggered() ), stWs, SLOT( tileSubWindows() ) );
  myWindowPopup->addAction( a );
...
{
  qDebug() << "Updating file actions!";
#if QT_VERSION >= QT_VERSION_CHECK(5,0,0)
  if ( stWs->subWindowList().isEmpty() )
#else
...
DocumentCommon* ApplicationCommonWindow::onNewDoc()
{
    ...
    connect( stWs, SIGNAL( subWindowActivated( QMdiSubWindow* ) ),
...
#if QT_VERSION >= QT_VERSION_CHECK(5,0,0)
    MDIWindow* m = (MDIWindow*)stWs->activeSubWindow()->widget();
#else
...
{
#if QT_VERSION >= QT_VERSION_CHECK(5,0,0)
    DocumentCommon* doc = ((MDIWindow*) stWs->activeSubWindow()->widget())->getDocument();
#else
...
{
  if ( w ) {
    ((MDIWindow*) w->widget())->onWindowActivated();
  }
}
...
void ApplicationCommonWindow::onSelectionChanged()
{
  ...
  QMdiArea* ws = ApplicationCommonWindow::getWorkspace();
  DocumentCommon* doc;
  MDIWindow *aMdiWindow = qobject_cast<MDIWindow*>( ws->activeSubWindow()->widget() );
  if( ! aMdiWindow)
	  return;
...
#endif
    qDebug() << "Getting a context from doc!" << endl;
    Handle(AIS_InteractiveContext) context = doc->getContext();
    qDebug() << "Got a context: " << context;
